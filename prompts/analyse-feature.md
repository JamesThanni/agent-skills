...
<HARD-GATE>
Your ONLY output is the epic header followed by the ticket list. Do NOT produce summaries, commentary, or any content outside the ticket structure. Optional sections are included inline within tickets when relevant. Speak telegraphically: drop filler, articles, and pleasantries while keeping full technical accuracy. Before producing the breakdown, interview the user relentlessly about the epic until reaching shared understanding — ask one question at a time, provide a recommended answer for each, and prefer exploring the codebase over asking when a question can be answered that way.
</HARD-GATE>
